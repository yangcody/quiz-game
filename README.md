...
- 대용량 데이터를 쓸 경우 JSON 파일을 여러 개로 분리하거나 하나씩 읽는 방식으로 처리해야 하나 lazy loading을 쓰면 로직이 복잡해지므로 본격적인 데이터 처리를 위해서는 데이터베이스를 사용하는 편이 바람직함
...
- quizzes는 퀴즈 콘텐츠 데이터로 복수개로 존재하므로 list로 구현하여 반복처리에 용이하도록 설계,
  best_score는 현재까지의 최고 점수에 대한 단일값이며 실행결과 또는 상태로서 빈번하게 변경되므로 별도로 설정
...
새로운 퀴즈를 추가합니다.
문제를 입력하세요: Python에서 문자열을 출력할 때 사용하는 함수는?
선택지 1: print
선택지 2: input
선택지 3: len
선택지 4: type
...
[1] Python의 창시자는?
[2] Python의 파일 확장자는?
[3] 리스트(List)의 특징으로 맞는 것은?
[4] 딕셔너리(dict)의 특징은?
[5] True 또는 False 값을 가지는 타입은?
[6] 함수 정의에 사용하는 키워드는?
[7] Python에서 문자열을 출력할 때 사용하는 함수는?
----------------------------------------
...
### 요구사항(정답 채점 방식이나 퀴즈 구조) 변경될 경우 수정을 위한 유지보수 설명
- main.py 파일에서 main() 내부는 특별히 수정할 만한 사항 없음
- 퀴즈 구조 변경이나 quiz.py 파일에서 Quiz 클래스 중 display() 메쏘드 수정 필요
- 그 외 모든 수정은 quiz_game.py 파일의 QuizGame 클래스를 수정해야 하는데 수정이 필요한 메쏘들을 설명하면,
  메뉴 구조 변경은 run() 수정하고 해당 메쏘드를 신규/추가, 정답 채점이나 점수 계산은 play_quiz() 수정,
  퀴즈 구조 변경되면 get_answer_input(), add_quiz() 수정이 필요함
